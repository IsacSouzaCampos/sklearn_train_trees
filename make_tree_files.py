...
for path in os.listdir(dir_path):
	...
	np.savetxt(f'tree_files/{c50f_data}', train_data, fmt='%c', delimiter=',')
	np.savetxt(f'tree_files/{c50f_test}', test_data, fmt='%c', delimiter=',')
	save_names_file(nfeat=train_data.shape[1]-1, filename=f'tree_files/{c50f_names}')
